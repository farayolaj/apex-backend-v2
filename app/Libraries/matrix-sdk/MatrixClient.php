...
use MatrixSdk\Crypto\OlmDevice;
use MatrixSdk\Exceptions\MatrixException;
use MatrixSdk\Exceptions\MatrixHttpLibException;
use MatrixSdk\Exceptions\MatrixUnexpectedResponse;
use MatrixSdk\Exceptions\ValidationException;
...
class MatrixClient
{
    ...
    public function __construct(
        ...
        if ($useToken) {
            ...
            try {
                ...
                if (!$token) {
                    log_message("warning", "[Matrix] original token {$useToken} failed: {$e->getMessage()} attempting to login using stored credentials for user " . env('MATRIX_USERNAME'));
    ...
    public function sync(int $timeoutMs = 30000)
    {
        ...
        if ($this->encryption && $response->has('device_one_time_keys_count')) {
