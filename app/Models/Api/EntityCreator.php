...
class EntityCreator
{
    ...
    private function uploadFile(string $model, string $name, $type, $maxSize, $destination, &$message = '', $insertType = false, $preserve = false, int $max_width = 0, int $max_height = 0): bool
    {
        ...
        if (!empty($filename) && $typeValid && !empty($destination)) {
            ...
            $uploadDestination = $this->_rootUploadsDirectory . $destination;
            ...
            $tmpFilename = str_replace([" ", '/'], ['_'], pathinfo($filename, PATHINFO_FILENAME)) . "_" . date('Y-m-d-H:i:s');
            if ($model == 'student_verification_documents') {
                $academicRecord = $user->academic_record;
                $newName = str_replace([" ", '/'], ['_'], $academicRecord->application_number ?: $academicRecord->matric_number);
                if (isset($parameter['verification_documents_requirement_id']) && $parameter['verification_documents_requirement_id']) {
                    $verificationModel = loadClass('verification_documents_requirement');
                    $verificationDoc = $verificationModel->verification_documents_requirement->getWhere(['id' => $parameter['verification_documents_requirement_id']], $count, 0, null, false);
                    if ($verificationDoc) {
                        $tmpFilename = str_replace(" ", '_', strtolower($verificationDoc[0]->name));
                    }
                }
            } else {
                $newName = $user->user_login;
            }
            // $newName.= "_".$tmpFilename."_".date('Y-m-d-H:i:s').".".$ext;
            $newName .= "_" . $tmpFilename . "." . $ext;
            if ($insertType) {
                $getUpload = $this->getUploadID($model, $insertType, $name);
                $naming = ($getUpload === 'insert') ? $newName : pathinfo($getUpload, PATHINFO_BASENAME);
            }
            $uploadDestination .= $naming;
            if (move_uploaded_file($_FILES[$name]['tmp_name'], $uploadDestination)) {
                $message = $destination;
                return true;
            } else {
    ...
    /**
     * The message variable will give the error message if there is an error and the variable is passed
     * @param string $model
     * @param string $type
     * @param array $data
     * @param $db
    ...
    private function insertSingle(string $model)
    {
        ...
        if ($newModel->insert($this->db, $message)) {
            ...
            if ($this->DoAfterInsertion($model, 'insert', $parameter, $this->db, $message, $extra)) {
                ...
                return ApiResponse::success($message, $inserted);
            }
        }
        $this->db->transRollback();
        $this->removeFileUpload($model, $parameter);
    ...
    /**
     * This removes file uploaded in case of an error
     * @param string $model [description]
     * @param array $parameter [description]
     * @return array|void [type]            [description]
     */
    private function removeFileUpload(string $model, array $parameter)
    {
        if (!empty($_FILES)) {
            $paramFile = $model::$documentField;
            $fields = array_keys($_FILES);
    ...
    private function updateSingle(string $model, int $id, $param = false)
    {
        ...
        if ($tempParameter = $this->createModelTimestamp($newModel, $model, 'update')) {
            if (!empty($tempParameter)) {
                $parameter = array_merge($data, $tempParameter);
            }
        }
        $this->db->transBegin();
        $parameter['id'] = $id;
    ...
    {
        if (!$this->isModel($model)) {
            return ApiResponse::error('error occurred while deleting information');
        }
    }
