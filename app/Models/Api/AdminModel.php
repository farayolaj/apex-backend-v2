...
class AdminModel
{
    ...
    {
        $zip = \CodeIgniter\Config\Factories::libraries('Zip');
    ...
    private function loadPhotosByFilter($search, $session, $entry, $level, $dept, $program, $course, $page = false, $pageSize = false, $returnPath = true): array
    {
        ...
        if ($session) {
            ...
            $param[] = $this->db->escapeString($session);
        }
        if ($entry) {
            $where .= ($where ? ' and ' : ' where ') . " b.session_of_admission=?";
            $param[] = $this->db->escapeString($entry);
        }
        if ($level) {
            if ($session) {
            ...
            $param[] = $this->db->escapeString($level);
        }
        if ($dept) {
            $where .= ($where ? ' and ' : ' where ') . " f.id=?";
            $param[] = $this->db->escapeString($dept);
        }
        if ($program) {
            if ($session) {
            ...
            $param[] = $this->db->escapeString($program);
        }
        if ($course) {
            $where .= ($where ? ' and ' : ' where ') . " c.course_id=?";
            $param[] = $this->db->escapeString($course);
        }
        if ($search) {
    ...
    private function getCourses($userType, $session, $level, $program): array
    {
        ...
        $courses = $courses->getResultArray();
        if (!$courses) {
            return [];
        }
    ...
    private function getCourseResultInfo($course, $session, $level, $program)
    {
        ...
        $query = "select count(DISTINCT CASE WHEN course_enrollment.total_score IS NOT NULL THEN course_enrollment.student_id END) as scored,count(if(total_score=null,1,null)) as unscored,min(total_score) as min_score,max(total_score) as max_score, avg(total_score) as avg_score from course_enrollment join academic_record on academic_record.student_id=course_enrollment.student_id $extra ";
        $result = $this->db->query($query, $param);
        $result = $result->getResultArray();
        return $result[0];
    }
    ...
    private function getTotalStudentRegistration($session, $semester = false, $level=null, $program=null)
    {
        $extra = '';
        $param = [];
        if ($session) {
    ...
    private function loadPrograms()
    {
        ...
        $tempRes = $tempRes->getResultArray();
     ...
     * @param mixed $program
     */
    public function getActiveStudentCount($session = false, $level = false, $program=null)
    {
        $extra = '';
        $param = [];
        if ($session) {
    ...
    {
        $applicantModel = \CodeIgniter\Config\Factories::models('ApplicantModel');
        return $applicantModel->getApplicantStatsData('application', $session);
    }
    ...
    {
        $applicantModel = \CodeIgniter\Config\Factories::models('ApplicantModel');
        return $applicantModel->getApplicantStatsData('registered', $session);
    }
    ...
    {
        $applicantModel = \CodeIgniter\Config\Factories::models('ApplicantModel');
        return $applicantModel->getApplicantStatsData('admitted', $session);
    }
    ...
    {
        $applicantModel = \CodeIgniter\Config\Factories::models('ApplicantModel');
        return $applicantModel->getApplicantStatsData('accepted', $session);
    }
    ...
    private function totalPassiveStudent($session = null)
    {
        ...
        $result = $query->getResultArray()[0]['total'];
    ...
    private function totalGraduatedStudent()
    {
        if (isset($_GET['dashboard_department'])) {
            $department = $_GET['dashboard_department'];
            $query = "SELECT count(distinct b.student_id) as total from students a join academic_record b on b.student_id = a.id right join
				programme c on c.id = b.programme_id right join department d on d.id = c.department_id where a.active = '2' and d.id = '$department' ";
        } else {
            $query = "SELECT count(*) as total from students a where a.active = '2' ";
        }
        $query = $this->db->query($query);
        $result = 0;
        if ($query->getNumRows() <= 0) {
    ...
    public function getStudentWithEnrollment($session, $semester)
    {
        ...
        return $result->getResultArray()[0]['total'];
    }
    ...
    private function getDistinctCourseCodeWithScore($session, $semester)
    {
        if (isset($_GET['dashboard_department'])) {
            $department = $_GET['dashboard_department'];
            $query = "SELECT distinct a.course_id, c.code as course_code, count(distinct a.student_id) as total,
    ...
    public function getTopData(): array
    {
        ...
        return [
            ...
            'graduated_student' => (int)$this->totalGraduatedStudent(),
        ];
    }
    ...
    private function processStudentDepartmentLevel($session)
    {
        ...
        if ($semester) {
            $query = "SELECT distinct e.name,f.slug as faculty,a.level,e.slug as department, 
				CONCAT(
        			 COALESCE(COUNT(DISTINCT CASE WHEN a.payment_id = '1' THEN a.student_id END), 0),
    					'|',
    				 COALESCE(COUNT(DISTINCT CASE WHEN a.payment_id = '2' THEN a.student_id END), 0)
    			) AS total,
    			COALESCE(COUNT(DISTINCT CASE WHEN a.payment_id = '1' THEN a.student_id END), 0) AS semester_1_total,
    			COALESCE(COUNT(DISTINCT CASE WHEN a.payment_id = '2' THEN a.student_id END), 0) AS semester_2_total
				from transaction a 
				left join academic_record c on c.student_id = a.student_id 
				left join programme d on d.id = c.programme_id 
				left join department e on e.id = d.department_id 
				left join faculty f on f.id = d.faculty_id where a.payment_id in ('1', '2') 
				and a.payment_status in ('01', '00') and a.session = ? and e.type='academic' 
				group by e.name,f.slug,a.level,e.slug order by f.slug, e.name asc";
        } else {
            $query = "SELECT distinct e.name,f.slug as faculty,a.level,e.slug as department, count(a.student_id) as total 
				from transaction a right join academic_record c on c.student_id = a.student_id 
				right join programme d on d.id = c.programme_id 
				right join department e on e.id = d.department_id 
				right join faculty f on f.id = d.faculty_id where a.payment_id = '1' and 
				a.payment_status in ('01', '00') and a.session = ? and e.type='academic' 
				group by e.name,f.slug,a.level,e.slug order by f.slug, e.name asc";
        }
        $query = $this->db->query($query, [$session]);
        $result = [];
    ...
    {
        $session = request()->getGet('session') ?: $this->currentSession();
    ...
    private function processByDepartmentInFaculty(array $data): array
    {
        ...
        foreach ($data as $key => $values) {
            ...
            foreach ($values as $val) {
                $newLevel = formatStudentLevel($val['level']);
                $payload = [
                    'level' => $newLevel,
                    'total' => $val['total'],
                ];
    ...
    {
        return [100, 200, 300, 400, 500, 401, 402, 501, 502];
    }
    ...
    private function fillMissingLevelWithNil(array $data = []): array
    {
        ...
        foreach ($data as $key => $values) {
            ...
            if (!empty($differences)) {
                $content = [];
                foreach ($differences as $item) {
                    $newLevel = formatStudentLevel($item);
                    $payload = [
    ...
    public function getFinanceOutflowPayment()
    {
        ...
        $query = $this->db->query($query, [OutflowStatus::SUCCESSFUL->value, $session]);
        return $query->getResultArray()[0]['total'];
    }
    ...
    public function getDashboardLatestTransactionDebit($limit = 25): array
    {
        ...
        foreach ($result as $item) {
            ...
            if ($item['payment_status_description']) {
                $temp['payment_status'] = $item['payment_status_description'] === OutflowStatus::SUCCESSFUL->value ? '00' : '021';
            } else {
    ...
    public function loadScoreAnalysisOld($session, $course)
    {
        ...
        $query = "SELECT
			...
			END AS scored,
       		count(if(total_score=null,1,null)) as unscored, min(total_score) as min_score,
       		max(total_score) as max_score, avg(total_score) as avg_score 
			from course_enrollment a 
       		join academic_record b on b.student_id=a.student_id 
			LEFT JOIN course_manager c ON c.course_id = a.course_id AND c.session_id = a.session_id
    ...
    public function loadScoreAnalysis($session, $course)
    {
        ...
        $query = "SELECT
    			...
    			END AS scored,
				 ...
				 CASE
					WHEN a.session_id >= $sessionCondition THEN
						COUNT(DISTINCT a.student_id) - COALESCE(MAX(c.total_graded), 0)
					ELSE
						COUNT(DISTINCT IF(a.total_score IS NULL, a.student_id, NULL))
				END AS unscored,
    ...
    {
        $sessionCondition = SettingSlug::SESSION_GRADED_START;
        $query = "SELECT total_score as label,
			CASE
				WHEN a.session_id >= $sessionCondition THEN
					MAX(0)
				ELSE
    ...
    public function loadAvgCourseScoreBySession($session, $course)
    {
        if ($session >= SettingSlug::SESSION_GRADED_START) {
            $query = "SELECT COALESCE(avg(b.total_graded), 0) as total from course_enrollment a 
            	LEFT JOIN course_manager b on b.course_id = a.course_id and b.session_id = a.session_id 
            	where a.course_id = ? and a.session_id = ?";
        } else {
            $query = "SELECT COALESCE(avg(total_score), 0) as total from course_enrollment a where a.course_id = ? and a.session_id = ?";
        }
        $result = $this->db->query($query, [$course, $session]);
        return $result->getResultArray()[0]['total'];
    }
    ...
    public function loadCourseEnrollmentScoreTrend($session, $course): array
    {
        ...
        $session = $session[0]['date'];
        $maxYear = (int)substr($session, 0, 4);

        $query = "SELECT a.session_id, b.date as label 
          FROM course_enrollment a JOIN sessions b ON b.id = a.session_id WHERE a.course_id = ? 
          AND SUBSTRING(b.date, 1, 4) + 0 <= ?
          GROUP BY a.session_id, b.date
          ORDER BY SUBSTRING(b.date, 1, 4) DESC
          LIMIT 5";
        $result = $this->db->query($query, [$course, $maxYear]);
        $result = $result->getResultArray();
        $payload = [];
        if (!empty($result)) {
    ...
    public function totalBookstoreSales(string $type = 'total')
    {
        if ($type === CommonSlug::GES_BOOK->value) {
            $bookType = CommonSlug::GES_BOOK->value;
            $query = "SELECT COALESCE(sum(distinct a.total_amount), 0) as total_amount,
       		COALESCE(sum(c.quantity), 0) as qty from student_payment_bookstore a join 
			transaction b on b.transaction_ref = a.transaction_ref 
        	join student_payment_bookstore_items c on c.student_payment_bookstore_id = a.id 
        	where b.payment_status in ('00', '01') and c.book_type = '$bookType' ";
        } else if ($type === CommonSlug::DLC_BOOK->value || $type === CommonSlug::COURSE_PACK->value) {
            $bookType = $type === CommonSlug::DLC_BOOK->value ? CommonSlug::DLC_BOOK->value : CommonSlug::COURSE_PACK->value;
            $query = "SELECT COALESCE(sum(distinct a.total_amount), 0) as total_amount,
       			COALESCE(sum(c.quantity), 0) as qty from student_payment_bookstore a join 
			transaction b on b.transaction_ref = a.transaction_ref 
        	join student_payment_bookstore_items c on c.student_payment_bookstore_id = a.id 
			...
			transaction b on b.transaction_ref = a.transaction_ref 
        	join student_payment_bookstore_items c on c.student_payment_bookstore_id = a.id 
        	where b.payment_status in ('00', '01') ";
        }
        $result = $this->db->query($query);
        return $result->getResultArray()[0];
    }

    public function totalInventoryStats()
    {
        $query = "SELECT COALESCE(sum(a.quantity), 0) as total_quantity, COALESCE(sum(a.price), 0) as total_amount 
