 ...
 */
class Applicant_post_utme extends Crud
{
	protected static $tablename = 'Applicant_post_utme';
...
//the folder to save must represent a path from the basepath. it should be a relative path,preserve filename will be either true or false. when true,the file will be uploaded with it default filename else the system will pick the current user id in the session as the name of the file.
	...
	static $tableAction = array('delete' => 'delete/applicant_post_utme', 'edit' => 'edit/applicant_post_utme');
...
<textarea id='user_agent' name='user_agent' class='form-control' required>$value</textarea>
	 ...
	 * @return null|<missing>
	 */
	protected function getApplicantTransaction(): ?array
	{
		return [[
			'id' => '01',
			'payment_id' => '1',
			'payment_description' => 'Application Processing Fee',
			...
			'beneficiary_1' => '100',
			'beneficiary_2' => '100',
			'payment_status_description' => 'Approved',
			'amount_paid' => '0',
			'service_charge' => '0',
			'total_amount' => '0',
			'date_performed' => $this->date_created,
			'date_completed' => $this->date_created,
			'date_payment_communicated' => $this->date_created,
			'transaction_id' => '',
			'subaccount_amount' => '0',
			'mainaccount_amount' => '0',
			'beneficiary_3' => '',
		]];
	}
	...
	{
		$query = "SELECT * from applicant_post_utme where applicant_id = ? or email = ? ";
	...
	{
		$query = "SELECT * FROM applicant_transaction WHERE applicant_id=? and payment_status in ('00','01')";
	...
	public function APIList($filterList, $queryString, $start, $len, $orderBy): array
	{
		...
		c.name as programme_offered,d.name as admission_type from applicants a left join programme b on b.id = a.programme_id
		left join programme c on c.id = a.programme_given left join admission d on d.id = a.admission_id $filterQuery";
	...
	public function applicationDocument($applicant_id)
	{
        ...
        d.rrr_code, d.payment_status as payment_status_code,d.payment_status_description,b.name as programme, p2.name as programme_name_given,
        c.date as session_date, d.amount_paid, d.total_amount,d.date_performed as date_initiated,d.date_completed from applicant_post_utme a left join
    	programme b on b.id = a.programme_id left join programme p2 on p2.id = a.programme_given left join
    	sessions c on c.id = a.session_id left join applicant_transaction d on d.applicant_id = a.id
		...
		return new Applicant_post_utme($result[0]);
	}
