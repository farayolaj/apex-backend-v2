...
class Examination_courses extends Crud
{
    ...
    public function loadExtras($item, $currentUser)
    {
        ...
        $item['action_url'] = [
            'view_scores' => base_url('web/examination_scores_list/' . hashids_encrypt($item['course_id']) . '/' . hashids_encrypt($item['session']) . '/' . hashids_encrypt($item['student_level']) . '/' . hashids_encrypt($item['course_manager_id'])),
        ];
        if ($item['course_manager_id'] == $currentUser->id) {
            $item['can_upload'] = true;
            $item['action_url'] = [
                'view_scores' => base_url('web/examination_scores_list/' . hashids_encrypt($item['course_id']) . '/' . hashids_encrypt($item['session']) . '/' . hashids_encrypt($item['student_level']) . '/' . hashids_encrypt($item['course_manager_id'])),
                'enter_scores' => base_url('web/examination_scores/' . hashids_encrypt($item['course_id']) . '/' . hashids_encrypt($item['session']) . '/' . hashids_encrypt($item['student_level']) . '/' . hashids_encrypt($item['course_manager_id'])),
                'bulk_upload' => base_url('web/examination_scores/' . hashids_encrypt($item['course_id']) . '/' . hashids_encrypt($item['session']) . '/' . hashids_encrypt($item['student_level']) . '/' . hashids_encrypt($item['course_manager_id'])),
                'bulk_sample_upload' => base_url('web/download_result_sample/' . hashids_encrypt($item['course_id']) . '/' . hashids_encrypt($item['session']) . '/' . hashids_encrypt($item['student_level']) . '/' . hashids_encrypt($item['course_manager_id'])),
            ];
        }
